 ...
 * Mongo: deviceType AQID + category monitoring
 *
 * Serial 115200:
 *   t  → high temperature
 *   h  → high humidity
 *   a  → high AQI (alert test)
 *   n  → normal values
 *   s  → send now
 */
...
// LOCAL Mongo: deviceType AQID, category monitoring
const char* deviceId = "YOUR_AQID_DEVICE_ID";
...
enum ForceMode { FORCE_NONE, FORCE_TEMP, FORCE_HUM, FORCE_AQI };
...
void publishSensorData() {
  ...
  int aqi = 20 + random(0, 40);   // ~20–59 normal-ish

  if (forceMode == FORCE_TEMP) {
    temperature = 45.0 + random(0, 50) / 10.0;
  } else if (forceMode == FORCE_HUM) {
    humidity = 90.0 + random(0, 50) / 10.0;
  } else if (forceMode == FORCE_AQI) {
    aqi = 180 + random(0, 50);    // high AQI
  }

  StaticJsonDocument<192> doc;
  doc["temperature"] = temperature;
  doc["humidity"] = humidity;
  doc["AQI"] = aqi;               // exact key: "AQI" (conditionChecker / processor)
...
void handleSerial() {
  ...
  if (c == 't' || c == 'T') {
    ...
    Serial.println("Keys: t temp | h hum | a AQI | n normal | s send");
  }
}
...
void reconnect() {
  ...
  String clientId = "ESP32-AQID-" + String(deviceId);
